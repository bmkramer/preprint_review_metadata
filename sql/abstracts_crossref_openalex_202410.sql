...
FROM `[project].crossref_metadata.crossref_metadata20241031`),
...
FROM `[project].openalex.works` as papers ---data snapshot 20241030
---FROM `[project].openalex_snapshot.works20241030` as papers
WHERE papers.doi IS NOT NULL
GROUP BY doi) as dois

LEFT JOIN (SELECT id, abstract_inverted_index FROM `[project].openalex.works`) as papers ON papers.id = dois.id ---data snapshot 20241030
--- LEFT JOIN `[project].openalex_snapshot.works20241030` as papers ON papers.id = dois.id
),
...
--- can also include abstract length
...
a.*,
ROUND(SAFE_DIVIDE(a.crossref_has_abstract, crossref_dois), 3) as pc_crossref_abstracts,
ROUND(SAFE_DIVIDE(a.openalex_has_abstract, crossref_dois), 3) as pc_openalex_abstracts,
b.member_primary_name as publisher_name

FROM TABLE_AGG as a
LEFT JOIN `[project].crossref.member_data_20241120` as b
...
SELECT * FROM table_final LIMIT 1000
